    ...
    fs::read_dir(&versions_dir)
        .map(|entries| {
            entries
                ...
                .filter(|e| {
                    let jar = e.path().join(format!("{}.jar", e.file_name().to_string_lossy()));
                    jar.exists()
                })
...
pub async fn install_version(
    ...
    fs::create_dir_all(&version_dir).map_err(|e| format!("Ошибка создания директории: {}", e))?;

    // 1. Скачиваем метаданные версии
    emit_progress(&window, "Загрузка метаданных", 0.0, &version_id);
    ...
    let meta_json = client.get(&version_url).send().await.unwrap().text().await.unwrap();
    fs::write(version_dir.join(format!("{}.json", version_id)), &meta_json)
        .map_err(|e| format!("Ошибка сохранения: {}", e))?;

    // 2. Скачиваем клиент .jar
    emit_progress(&window, "Загрузка клиента", 0.05, &format!("{}.jar", version_id));
    ...
    for (i, lib) in meta.libraries.iter().enumerate() {
        ...
        emit_progress(&window, "Загрузка библиотек", progress, &lib.name);
    ...
    if let Some(asset_index) = &meta.asset_index {
        emit_progress(&window, "Загрузка ассетов", 0.7, &asset_index.id);
        ...
        if let Ok(asset_index_file) = serde_json::from_str::<AssetIndexFile>(&index_text) {
            ...
            for (_name, obj) in &asset_index_file.objects {
                ...
                if !obj_path.exists() {
                    ...
                    // Пропускаем ошибки для отдельных ассетов
                    if let Err(_) = download_file(&client, &url, &obj_path).await {
                        continue;
                    }
                }
    ...
    emit_progress(&window, "Готово", 1.0, &format!("Версия {} установлена!", version_id));
...
pub fn delete_version(version_id: String) -> Result<(), String> {
    ...
    if version_dir.exists() {
        fs::remove_dir_all(&version_dir).map_err(|e| format!("Ошибка удаления: {}", e))?;
    }
    Ok(())
}
...
pub async fn launch_game(
    ...
    // Определяем natives dir
    let natives_dir = version_dir.join("natives");
    fs::create_dir_all(&natives_dir).ok();

    // JVM аргументы
    let java_cmd = if java_path.is_empty() { "java".to_string() } else { java_path };
    ...
    // ely.by authlib inject для скинов (указываем authserver)
    // Добавляем в начало JVM аргументов
    args.insert(0, "-Dminecraft.api.auth.host=https://authserver.ely.by/auth".into());
    args.insert(1, "-Dminecraft.api.account.host=https://authserver.ely.by/account".into());
    args.insert(2, "-Dminecraft.api.session.host=https://authserver.ely.by/session".into());
    args.insert(3, "-Dminecraft.api.services.host=https://authserver.ely.by".into());

    emit_progress(&window, "Запуск", 1.0, &format!("Minecraft {} для {}", version_id, username));
    ...
    match cmd.spawn() {
        Ok(_child) => Ok(format!("Minecraft {} запущен для {}", version_id, username)),
        Err(e) => Err(format!("Ошибка запуска: {}. Проверьте путь к Java: {}", e, java_cmd)),
    }
}
...
pub fn save_config(config: LauncherConfig) -> Result<(), String> {
    let config_path = get_game_dir().join("launcher_config.json");
    fs::create_dir_all(get_game_dir()).ok();
...
fn emit_progress(window: &tauri::Window, stage: &str, progress: f64, detail: &str) {
    let _ = window.emit(
        "download-progress",
        DownloadProgress {
            stage: stage.into(),
